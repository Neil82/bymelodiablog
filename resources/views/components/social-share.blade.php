@props(['url' => null, 'title' => null, 'description' => null, 'class' => ''])

@php
    $shareUrl = $url ?: request()->url();
    $shareTitle = $title ?: 'Échale un vistazo a esto en ByMelodia';
    $shareDescription = $description ?: 'Contenido increíble sobre cultura juvenil';
    $encodedUrl = urlencode($shareUrl);
    $encodedTitle = urlencode($shareTitle);
    $encodedDescription = urlencode($shareDescription);
@endphp

<div class="social-share {{ $class }}">
    <div class="flex items-center space-x-3">
        ...
        <a href="https://t.me/share/url?url={{ $encodedUrl }}&text={{ $encodedTitle }}" 
           target="_blank" 
           rel="noopener noreferrer"
           class="flex items-center justify-center w-10 h-10 bg-blue-500 hover:bg-blue-600 text-white rounded-full transition-colors duration-200"
        ...
        <button onclick="copyToClipboard('{{ $shareUrl }}')" 
                class="flex items-center justify-center w-10 h-10 bg-gray-500 hover:bg-gray-600 text-white rounded-full transition-colors duration-200"
                title="Copiar enlace">
